...
<body class="font-sans antialiased bg-gray-50 text-gray-900 dark:bg-gray-900 dark:text-gray-100">
    <div class="min-h-screen flex flex-col">
        ...
        <!-- Page Content -->
        <main class="flex-1 max-w-7xl mx-auto w-full py-8 px-4 sm:px-6 lg:px-8 pb-24">
            {{ $slot }}
        </main>
    </div>

  <!-- Floating Button Container -->
<div class="fixed bottom-4 left-0 w-full flex justify-between items-center px-4 z-50 space-x-2 sm:space-x-4">

    @auth
        <!-- Back to Home -->
        <a href="{{ url('/') }}"
           class="inline-flex items-center px-3 py-2 text-xs sm:text-sm font-medium text-indigo-600 bg-white border border-indigo-600 rounded-full shadow-md hover:bg-indigo-50 dark:bg-gray-800 dark:text-gray-100 dark:border-indigo-400 dark:hover:bg-gray-700 transition">
            ⬅️ Home
        </a>

        <!-- Theme Toggle -->
        <button id="theme-toggle"
            class="inline-flex items-center justify-center px-3 py-2 text-xs sm:text-sm font-medium rounded-full bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200 shadow-md hover:bg-gray-300 dark:hover:bg-gray-600 transition">
            🌞 / 🌙
        </button>

        <!-- Dashboard -->
        <a href="{{ route('dashboard') }}"
           class="inline-flex items-center px-3 py-2 text-xs sm:text-sm font-medium text-white bg-indigo-600 rounded-full shadow-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition">
            Dashboard
        </a>
    @endauth
</div>
